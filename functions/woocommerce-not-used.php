...
add_action( 'admin_menu', 'kno_woocommerce_admin_menu', 999 );
function kno_woocommerce_admin_menu() {
    global $menu;
    $woo = recursive_array_search_php( 'WooCommerce', $menu );
    if( !$woo )
        return;
    $menu[$woo][0] = __( 'Obchod', 'kno' );
    $menu[$woo][4] = 'menu-top menu-icon-generic-fake toplevel_page_woocommerce';
    $menu[$woo][6] = 'dashicons-cart';
}

function recursive_array_search_php( $needle, $haystack ) {
    foreach( $haystack as $key => $value ) {
        $current_key = $key;
        if ( $needle === $value OR ( is_array( $value ) && recursive_array_search_php( $needle, $value ) !== false ) ) {
            return $current_key;
        }
    }
    return false;
...
add_action( 'product_cat_edit_form_fields', 'kno_product_cat_edit_form_fields', 10, 2);
function kno_product_cat_edit_form_fields( $term, $taxonomy ) {
    ?>
    <tr valign="top">
...
function kno_woocommerce_product_single_add_to_cart_text( $default ) {
    return __( 'Přidat do poptávky', 'kno' );
}
...
function kno_get_price_api( $price, $product ) {
    ...
    write_log('dotazuju se na cenu produktu ' .  $product->get_id() . ' s cenou ' . $price );
    ...
    $response = wp_remote_post( 'https://kalkulacka.kolonaoperak.cz/api/calculation', array(
        'headers'     => array('Content-Type' => 'application/json; charset=utf-8'),
        'body'        => json_encode( array( 'productPrice' => round( $price ) ) ),
        'method'      => 'POST',
        'data_format' => 'body',
    ) );
    ...
    if ( ( isset( $_GET['aktualizace-cen'] ) && $_GET['aktualizace-cen'] = 'dsf56vy46y4asd8' ) || ( is_admin() && isset( $_GET['aktualizace-cen'] ) ) ) {
        ...
        foreach( $query as $row ) {
            if ( !isset( $cache[ (float)$row->meta_value ] ) ) {
                ...
                if ( $product ) {
                    $tax_rates = WC_Tax::get_base_tax_rates( $product->get_tax_class( 'unfiltered' ) );
                    $taxes = WC_Tax::calc_tax( (float)$row->meta_value, $tax_rates, true );
                    $price = WC_Tax::round( (float)$row->meta_value * 1 - array_sum( $taxes ) );
...
add_action( 'admin_bar_menu', 'kno_api_prices_link', 999 );
function kno_api_prices_link( $wp_admin_bar ) {
    $args = array(
        'id' => 'kno_api',
...
*/
    ...
    if ( is_singular( 'product' ) && empty( wc_get_loop_prop( 'name' ) ) )
        $price .= '<div class="tooltip">?<span>' . __( 'Cena zahrnuje splátku a pojištění kola', 'kno' ) . '</span></div>';

    $price .= '<div class="price-item-perex">' . __( 'měsíční splátka na 24 měsíců', 'kno' ) . '</div>';
...
function kno_woocommerce_after_add_to_cart_button() {
    ...
    <div class="bubble product-bubble flex middle-xs">
        <div class="flex middle-xs"><strong><?php _e( 'Tento výrobek máme skladem', 'kno' ); ?></strong><?php _e( 'expedujeme do 24 hodin', 'kno' ); ?></div>
    </div>
<?php }
...
function print_attribute_radio( $checked_value, $value, $label, $attribute_name, $counter ) {
    ...
    printf( '
        <div class="radio">
            <label>
                <input type="radio" name="%1$s" value="%2$s" id="%3$s" %4$s data-validate-required-message="' . __( 'Výběr varianty je povinný.', 'kartonara' ) . '" required aria-label="' . __( 'Výběr varianty', 'kartonara' ) . '" /><i class="helper">%5$s</i>
            </label>
        </div>',
...
function kno_woocommerce_gettext( $translated_text, $text, $domain ) {
    switch ( $translated_text ) {
        case 'Aktualizovat košík' :
            $translated_text = __( 'Aktualizovat poptávku', 'kno' );
            break;
...
function kno_facetwp_sort_options( $options, $params ) {
    ...
    $options['default']['label'] = __( 'Výchozí třídění', 'kno' );
    $options['popularity_new'] = [
        'label' => __( 'Seřadit podle oblíbenosti', 'kno' ),
        'query_args' => [
            'orderby' => 'post_views',
            'order' => 'DESC',
        ]
    ...
    $options['price_asc'] = [
        'label' => __( 'Cena od nejlevnějších', 'kno' ),
        'query_args' => [
    ...
    $options['price_desc'] = [
        'label' => __( 'Cena od nejdražších', 'kno' ),
        'query_args' => [
            'orderby' => 'meta_value_num',
            'meta_key' => '_price',
            'order' => 'DESC',
...
//add_action( 'pmxi_delete_post', 'kno_pmxi_delete_post', 10, 2 );
function kno_pmxi_delete_post( $ids = array(), $import = 0 ){
    ...
    foreach( $ids as $pid ) {
        ...
        foreach( $translations as $locale => $post_id ) {
            global $wpdb;
            $variations = $wpdb->get_results( $wpdb->prepare( "SELECT p.ID FROM {$wpdb->posts} AS p WHERE p.post_type = 'product_variation' AND p.post_parent = %d" ), $post_id );
            if ( !empty( $variations ) ) {
...
function kno_pmxi_before_delete_post( $post_id, $import ) {
...
//add_action( 'save_post', 'kno_save_post', 999, 3 );
function kno_save_post( $post_id, $post, $update ) {
...
*/
    ...
    remove_action( 'save_post', 'kno_save_post', 999, 3 );
...
add_action( 'wp', function() {
    if ( isset( $_GET['eea1270112ea7e694945f5539e7d8ace5f6508e8b246eab8fb5f2c9dbf42c025'] ) )  {
        kno_polylang_populate_smaller();
    }
});


//add_filter( 'pll_copy_taxonomies', 'kno_pll_copy_taxonomies', -1, 2 );
function kno_pll_copy_taxonomies( $taxonomies, $sync ) {
    return array_merge( $taxonomies, array( 'seller' ) );
}
...
function get_default_language_posts( $query ) {
	if ( $query->is_main_query() && function_exists('pll_languages_list') && !is_admin() ) {
...
function kno_polylang_populate_smaller() {
    ...
    update_option( 'import_lock', true, false );
    ...
    update_option( 'import_lock', false, false );

}

function kno_polylang_populate() {
    ...
    $query_without_lang = new WP_Query( array(
        'posts_per_page' => 50,
    ...
    if ( !empty( $query_without_lang->posts ) ) {
        foreach( $query_without_lang->posts as $post_id ) {
            pll_set_post_language( $post_id, pll_default_language() );
        }
    }

    $query_without_lang_check = new WP_Query( array(
        'posts_per_page' => -1,
        'post_type'      => array( 'product', 'product_variation' ),
        'fields'         => 'ids',
        'post_status'    => array('publish'),
    ...
    if( !empty( $query_without_lang_check->posts ) ) {
        update_option( 'import_lock', false, false );
        return;
    }

    $query_without_translations = new WP_Query( array(
        'posts_per_page' => 500,
        'post_type'      => array( 'product', 'product_variation' ),
        'fields'         => 'ids',
        'post_status'    => array('publish'),
    ...
    if ( !empty( $query_without_translations->posts ) ) {
        foreach( $query_without_translations->posts as $post_id ) {
            wp_insert_term( $group = uniqid( 'pll_' ), 'post_translations', array( 'description' => maybe_serialize( array( pll_default_language() => $post_id ) ) ) );
            wp_set_object_terms( $post_id, $group, 'post_translations' );
        }
    }
    ...
    if( empty( $check->posts ) ) {
        ...
        $default_lang = get_term_by('slug', pll_default_language(), 'language');

        $rows = $wpdb->get_results( $wpdb->prepare( "
            SELECT $wpdb->posts.ID FROM $wpdb->posts
            LEFT JOIN $wpdb->term_relationships ON ($wpdb->posts.ID = $wpdb->term_relationships.object_id)
            LEFT JOIN $wpdb->term_relationships wtr ON ($wpdb->posts.ID = wtr.object_id)
            LEFT JOIN $wpdb->term_taxonomy wtt ON (wtr.term_taxonomy_id = wtt.term_taxonomy_id)
            WHERE
            $wpdb->term_relationships.term_taxonomy_id = %d AND
            ( wtt.taxonomy = 'post_translations' AND wtt.count < %d ) AND
            $wpdb->posts.post_type IN ( 'product' ) AND $wpdb->posts.post_status = 'publish' GROUP BY $wpdb->posts.ID LIMIT 0,10
        ", $default_lang->term_id, count( pll_languages_list() ) ) );
            ...
            foreach( $rows as $row ) {
                ...
                if( $product->is_type( 'simple' ) || $product->is_type( 'variable' ) ){
                    ...
                    foreach ( $langs as $lang ) {
                        ...
                        write_log( 'kopiruji - ' . $row->ID . ' do jazyka ' . $lang);
                        echo 'kopiruji - ' . $row->ID . ' do jazyka ' . $lang;
                        $new_id = $polylang->sync_post_model->copy_post( $row->ID, $lang, false );
                    }

                }
            }

            $polylang->sync_post_model->save_group( $row->ID, $langs );
...
//nefunguje, občas nenastaví CZ
add_action( 'pmxi_saved_post', 'kno_pmxi_saved_post', 10, 3 );
function kno_pmxi_saved_post( $post_id, $xml_node, $is_update ) {

    $logger = function($m) {printf("[%s] $m\n", date("H:i:s"));flush();};
    call_user_func( $logger, "Nastavuji jazyk " . strtoupper( pll_default_language() ) . ' v ID ' . $post_id );
    ...
    //global $polylang;
    //$polylang->sync_post_model->save_group( $post_id, array() );
    //pll_save_post_translations( array( 'cs' => $post_id ) );

    //wp_set_object_terms( $post_id, array(),  );

    /*
    if ( get_post_type( $post_id ) !== 'product' ) {
        foreach( pll_get_post_translations( $post_id ) as $locale => $remove_post_id ) {
            if ( $locale !== pll_default_language() ) {
                write_log( 'savedPost odstranuju v jazyku ' . $locale . ' - ' . $remove_post_id );
                wp_delete_post( $remove_post_id, true );
            }
        }
    }
    */
...
/*
    if ( get_post_type( $post_id ) === 'product_variation' ) {
        ...
        $post_id = empty( $wp_post->post_parent ) ? $post_id : $wp_post->post_parent;
        write_log( 'moznaparent? ' . $post_id );
    }
*/

    //global $polylang;
    //$polylang->sync_post_model->save_group( $post_id, array('cs') );
    ...
    if ( get_post_type( $post_id ) === 'product' && count( $translations ) == 3 ) {
        ...
        foreach( $langs as $lang ) {
            $polylang->sync_post_model->copy_post( $post_id, $lang, false );
        }

        $polylang->sync_post_model->save_group( $post_id, $langs );
...
function kno_wp_all_import_before_variable_product_import( $post_id ) {
    remove_action( 'transition_post_status', '_update_term_count_on_transition_post_status', 10 );
    remove_action( 'transition_post_status', '_update_posts_count_on_transition_post_status', 10 );
    remove_action( 'post_updated', 'wp_save_post_revision', 10 );
}
...
function kno_wp_all_import_variable_product_imported( $post_parent ) {

    if ( get_post_type( $post_parent ) !== 'product' )
        return;
    ...
    // nutnost, jelikož se jinak odstraní term na překlad "post_translations" kvůli sloupci count, který je 0 -- translated-object.php 235
    wp_defer_term_counting(false);

    $logger = function($m) {printf("[%s] $m\n", date("H:i:s"));flush();};

    foreach( pll_get_post_translations( $post_parent ) as $locale => $post_id ) {
        if ( $locale !== pll_default_language() ) {
            call_user_func( $logger, "Odstraňuji jazyk " . strtoupper( $locale ) . ' v ID ' . $post_id );
            wp_delete_post( $post_id, true );
        }
    }

    foreach( $langs as $lang ) {
        call_user_func( $logger, "Kopíruji do jazyka " . strtoupper( $locale ) . ' v ID ' . $post_id  );
        $new_id = $polylang->sync_post_model->copy_post( $post_parent, $lang, false );

    }
    $polylang->sync_post_model->save_group( $post_parent, $langs );

    wp_defer_term_counting(true);

}

add_action( 'admin_init', 'kno_testadmin_initd', 10 );
function kno_testadmin_initd(){
...
/*
    $should_delete = new WP_Query( array(
        'posts_per_page' => -1,
        'post_type'      => array('product_variation' ),
        'fields'         => 'ids',
        'post_status'    => array('any'),
        'lang'           => implode( ',', array_diff( pll_languages_list(), (array)pll_default_language() ) )
    ) );

    if ( !empty( $should_delete->posts ) ) {
        foreach( $should_delete->posts as $post_id ) {
            wp_delete_post( $post_id, true );
        }
    }

    //$polylang->sync_post_model->copy_post( 34463, 'en', true );
    ...
    foreach( $langs as $lang ) {
        $polylang->sync_post_model->copy_post( 34452, $lang, true );
    }
    */

    //$polylang->sync_post_model->save_group( 34452, $langs );
}

add_action( 'pmxi_update_product_variation', 'kno_pmxi_update_product_variationd', 10, 1 );
function kno_pmxi_update_product_variationd( $variation_id ){
    //write_log( 'varianta update - ' . $variation_id );

    //$parent = get_post( $variation_id );
    ...
    if ( !empty( $parent->post_parent ) ) {
        ...
        global $polylang;
        foreach ( $langs as $lang ) {
            $polylang->sync_post_model->copy_post( $parent->post_parent, $lang, true );
        }
    }
    */
...
add_action( 'pmxi_product_variation_saved', 'kno_pmxi_product_variation_saved', 10, 1 );
function kno_pmxi_product_variation_saved( $variation_id ){
    //write_log( 'varianta saved - ' . $variation_id );

}

add_action( 'pmxi_before_xml_import', 'kno_pmxi_before_xml_import' );
function kno_pmxi_before_xml_import( $import_id ) {
    update_option( 'import_lock', true, false );

    /*
    global $wpdb;
    $posts_ids = $wpdb->get_results( $wpdb->prepare( "SELECT post_id FROM " . PMXI_Plugin::getInstance()->getTablePrefix() . "posts WHERE import_id = %d" , $import_id ) );

    if ( ! empty( $posts_ids ) ) {
        foreach ( $posts_ids as $post ) {
            foreach( pll_get_post_translations( $post->post_id ) as $locale => $post_id ) {
                if ( $locale !== pll_default_language() ) {
                    write_log( 'odstranuji jazyk ' . $locale . ' - ' . $post_id );
                    wp_delete_post( $post_id, true );
                }
            }
            //global $polylang;
            //$polylang->sync_post_model->save_group( $post->post_id, array() );
        }
    }
    */
}
...
function kno_pmxi_after_xml_import( $import_id ) {
    ...
    if ( ! empty( $posts_ids ) ) {
        ...
        */
        ...
        global $polylang;
        foreach ( $posts_ids as $post ) {
            //write_log( 'after - ' . $post->post_id );


                /*
            if ( $post->post_id === 34452 ) {
                global $polylang;
                remove_filter( 'pll_sync_post_fields', 'kno_pll_sync_post_fields', 10 );
            ...
            */
            ...
            //wp_set_post_terms( (int) $post->post_id, pll_default_language(), 'language' );

            //pll_set_post_language( $post->post_id, pll_default_language() );
            ...
            //wp_set_post_terms( (int) $post_id->post_id, pll_default_language(), 'language' );
            //pll_set_post_language( $post_id->post_id, pll_default_language() );

            // TOO HEAVY => error 500
            /*
            $product = wc_get_product( $post_id->post_id );

            if( $product->is_type( 'simple' ) || $product->is_type( 'variable' ) ){

                foreach ( $langs as $lang ) {
                    $polylang->sync_post_model->copy_post( $post_id->post_id, $lang, true );
